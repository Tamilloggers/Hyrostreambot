...
async def is_user_joined(message: Message, lang) -> bool:
    try:
        user = await message._client.get_chat_member(config_dict['UPDATES_CHANNEL'], message.chat.id)
        if user.status == "BANNED":
            await message.reply_text(
                text=lang.BAN_TEXT.format(config_dict['OWNER_ID']),
        ...
        await message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("Jᴏɪɴ ɴᴏᴡ 🔓", url=f"https://t.me/{config_dict['UPDATES_CHANNEL']}")]]
            ),
        ...
        await message.reply_text(
            text=f"<i>Sᴏᴍᴇᴛʜɪɴɢ ᴡʀᴏɴɢ ᴄᴏɴᴛᴀᴄᴛ ᴍʏ ᴅᴇᴠᴇʟᴏᴘᴇʀ</i> <b><a href=fhttps://t.me/{config_dict['UPDATES_CHANNEL']}'>[ ᴄʟɪᴄᴋ ʜᴇʀᴇ ]</a></b>",
            parse_mode=ParseMode.HTML,
            disable_web_page_preview=True
        )
        return False
...
async def gen_link(m: Message, _id, name: list) -> tuple[InlineKeyboardMarkup, str]:
    ...
    page_link = f"{config_dict['URL']}watch/{_id}"
    stream_link = f"{config_dict['URL']}dl/{_id}"
...
async def is_allowed(message: Message) -> bool:
    if config_dict['ALLOWED_USERS']:
        user_id_str = str(message.from_user.id)
        if user_id_str not in config_dict['ALLOWED_USERS'] and message.from_user.username not in config_dict['ALLOWED_USERS']:
...
async def validate_user(message: Message, lang=None) -> bool:
    ...
    if config_dict['TOS'] and not await is_user_accepted_tos(message):
        return False
    ...
    if config_dict['FORCE_UPDATES_CHANNEL'] and not await is_user_joined(message, lang):
        return False
